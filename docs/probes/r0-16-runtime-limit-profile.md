...
`packages/contracts/src/frozen-runtime-limit-profile.ts` 冻结 25 个顶层字段 +
...
| R0-06 实测 | `attachmentQueueBytes` 1 MiB / `attachmentQueueFrames` 32（byte cap 边界 1,048,577、frame cap 边界 33 实测）；`streamFramePayloadBytes` 64 KiB（实测 ~65,537 B/frame）；`streamFramesPerSecond` 240（实测热 Session 80 fps 的 3× headroom） |
| a-priori 目标 | 其余全部字段（工程边界，rationale 在模块注释；R4 在矩阵最低硬件 fixture 上验证） |
...
- 延迟门槛 = spec 原值：输入 median 75 / p99 300 ms（RT-PERF-01）；输出 p95
  ...
  presentation p95 150 / p99 500 ms（RT-PERF-09）。
- 固定负载 = RT-PERF-08：10 Alive Session、热 producer 5 MiB/s、60 秒。
- 三类负载预算（RT-PERF-10）：activeVisible / activeHidden / electronClosed
  × { Renderer / Main / Daemon RSS、CPU、paint latency、durable lag、
  snapshot latency }。RSS / CPU / paint / lag 为 a-priori 目标值（唯一校准点：
  R0-06 固定负载下 Daemon RSS 增长 ~19 MB、event-loop p95 ≈ 11 ms），
  **R4 必须在矩阵最低硬件（M1 / 8 GiB）fixture 上验证**——本机 M5 Pro /
  48 GiB 高于地板。
