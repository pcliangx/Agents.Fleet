...
> Branch: `R0-03`。验证 RT-REC-07 / RT-REC-08 / RT-REC-12 / RT-LAUNCH-06..08 / RT-STATE-22..23 / RT-T-08 / RT-T-27
> 的关键假设: 当持有 node-pty master 的 Daemon 死亡时, Agent 子进程、进程组与孤儿进程的真实行为。
...
**结论(三模式完全一致): Daemon 以任意方式(正常退出 / SIGTERM / SIGKILL)死亡时, Agent 子进程都不会随之终止,
而是被 reparent 到 launchd(pid 1)成为仍运行但不可 attach 的孤儿, 同时收到一次 SIGHUP(PTY master 关闭)。**
因此 ADR-0001「不宣称进程跨 Daemon 崩溃存活」与 RT-REC-07「不得自动 spawn replacement」被实测证实为必要:
Daemon 崩溃后**必须**做孤儿探测(RT-REC-12), 且 PID 单独不安全, 必须用 `{pid, lstart}` 完整身份再识别。

附带一条供应链发现: **node-pty 1.1.0 的 prebuilt `spawn-helper` 以 mode `0644` 发布(无可执行位)**,
而 npm 11 的 allow-scripts 默认拦截其 lifecycle chmod, 导致 `posix_spawnp` 以 EACCES 失败 ——
Daemon 安装/升级流程必须显式校验并修复执行位(及签名/公证), 见 RT-DIST-01 / SV1-SUPPLY-02。
...
> signal 处理; SIGHUP 默认会终止多数进程, 但不能假定所有 Agent 都如此 —— 这是 R1/R2 runtime 需复核的项。

## node-pty session / 进程组语义

- 子进程是**自身 process group 的 leader**: `pgid == childPid`(实测)。
...
- **PTY master 关闭 → 内核投递 SIGHUP**(三模式均观测到)。这是 master 消失时子进程收到的信号。
- 但 **SIGHUP 不是可靠清理信号**: trapping 子进程收到 SIGHUP 后仍存活为孤儿。结论: 不能依赖「Daemon 死 → 子进程自动死」。
- 子进程的 stdout 写在 master 消失后报错(EIO/EPIPE), 可作为「Daemon 消失」的**可观测信号** ——
  inert bootstrap 可借此类 fd 消失作为自超时触发(RT-LAUNCH-06), 但 bootstrap **不得**假定它会随 Daemon 自动退出, 必须自带超时(RT-LAUNCH-06/07)。
...
- **PID 复用**: 400 个短命进程 churn 窗口内**未观察到**旧 PID 被复用(`pidReuseNotObservedInProbeWindow=true`)。
  但未观察到 ≠ 不可能 —— macOS PID 终会被复用, 故 `pid-alone` 仍不安全(显式标注为推断)。
...
## node-pty spawn-helper 供应链发现(RT-DIST-01 / SV1-SUPPLY-02)
...
- 正常安装靠 node-gyp 重编译或 prebuild 解压恢复 +x; 但 **npm 11 的 allow-scripts 默认拦截 lifecycle 脚本**,
  该恢复路径被切断 → `posix_spawnp` 以 EACCES 失败, PTY 完全无法 spawn。
- 实测修复: `chmod 0755 spawn-helper`(`helperModeBefore=0o644 → helperModeAfter=0o755`)。
- 推论: 发布清单必须把 `spawn-helper` 纳入 native artifact 校验(执行位 + 代码签名 + 公证),
  Daemon 安装/升级不得依赖 npm 生命周期, 必须显式校验并修复。

## 对 Reconciliation / 启动协议 / 分发的强制推论

除标注「推断」外, 均由本次实测直接支持。

1. Daemon 崩溃后**必须**执行 Reconciliation 并做孤儿探测(RT-REC-07/08); 孤儿探测按完整身份, 不只比 PID(RT-REC-12)。
...
5. Daemon 安装/升级必须显式校验 `spawn-helper` 执行位与签名(RT-DIST-01 / SV1-SUPPLY-02)。

## 证据与复现

- 探测脚本: [`r0-03/probe.mjs`](r0-03/probe.mjs)(orchestrator) + [`r0-03/daemon-worker.mjs`](r0-03/daemon-worker.mjs)(旧 Daemon, node-pty owner) + [`r0-03/agent-child.mjs`](r0-03/agent-child.mjs)(Agent, heartbeat + signal 日志)
- 证据: [`r0-03/evidence-daemon-crash.json`](r0-03/evidence-daemon-crash.json)(脱敏实测)
- 结构化 fixture: [`packages/testing/src/fixtures/daemon-crash-behavior.ts`](../../packages/testing/src/fixtures/daemon-crash-behavior.ts)(实测/推断标注, 喂 ProcessSupervisor #9/#10 与 Reconciliation)
- 复现: `node docs/probes/r0-03/probe.mjs docs/probes/r0-03/evidence-daemon-crash.json`
  (probe 自带 setup: 在 `$TMPDIR/r0-03-pty-env` 临时 `npm install node-pty@1.1.0` 并 chmod helper; repo 零新依赖, 不触发 packages/apps 的 node-pty guard。)

## 边界与后续

- 单 Host 单次采样; `SupportedPlatformMatrix`(R0-15)冻结后需在矩阵最低 macOS / 最低硬件上复测同一 fixture。
- agent-child 为 signal-trapping; **真实 Agent 的存活取决于其 SIGHUP/SIGTERM 处理**, 需 R1(Claude)/ R2(Codex) runtime 复核 —— fixture 已显式标注为推断。
- macOS `ps -o sess=` 返回 0; session 语义的更强证据需 `getsid()` 级直接观测(本探测用 pgid 已可落地停止语义, 未单独做)。
- PID 复用未在窗口内复现; 无法证伪, 设计仍按「pid-alone 不安全」处理。
- launchd KeepAlive 自动拉起 Daemon 后与孤儿进程的交互(ADR-0001: LaunchAgent 拉起 Daemon)**未在本次覆盖**; RT-REC-07 的完整闭环需后续原型(可作为 R0-03 follow-up 或并入 Daemon 集成切片)。
