...
- **SV1-SUPPLY-02**：构建在固定 toolchain 与 lockfile 下生成 native artifact，验证 package integrity、native architecture、最低系统版本和禁止的 install-time network / script；architecture、deployment target 与最低系统版本必须匹配 RT-DIST-08 的 `SupportedPlatformMatrix`。开发 / 测试依赖安装若为真实安全 fixture 执行上游 lifecycle script，只能使用绑定精确 package version、默认拒绝的 reviewed allowlist（reviewed = 经 spec requirement 引用 + 双轴 code review 批准的精确版本；allowlist 见 `pnpm-workspace.yaml` 的 `allowBuilds`，未列版本默认 fail-closed），并锁定执行该 script 的 runtime 与下载 artifact checksum（runtime checksum 进 `pnpm-lock.yaml` 的 `node@runtime:<version>`；下载 artifact checksum 固化于 `config/electron-artifact-checksums.json`，`pnpm verify:electron` 校验）；由此取得的 artifact 不得充当发布输入。发布构建必须从独立验证的离线 artifact 生成，不能执行依赖 lifecycle script 或在缺失时回退网络。任何下游 patch 都有 owner、来源、hash、回归 fixture 与删除条件。

> _Changelog (R0-22, [probe](../probes/r0-22-spawn-helper.md), issue #22)_：node-pty 1.1.0 prebuilt `spawn-helper` 出厂 mode `0644`（无可执行位），npm 11 `allow-scripts` 拦截其 lifecycle chmod → `pty.spawn` 抛 `posix_spawnp failed.`。已新增 spawn-helper 校验器（`packages/daemon/src/native-artifact/spawn-helper-verifier.ts`，纯逻辑）+ 采样器：显式校验执行位 / architecture / **deployment target（≤ `SupportedPlatformMatrix.minimumMacOSVersion`）** / 签名强度 / hash，仅当唯一问题是缺执行位时许可 `chmod 0755` 修复作为带 owner 的下游 patch（不依赖 npm lifecycle；patch 记录随 `repairSpawnHelper` 共置），架构不符 / deployment target 过高 / hash 篡改 / 签名不足一律 fail-closed。实测复现 pre-repair `posix_spawnp failed.`、post-chmod 真实 `pty.spawn` PASS（副本上证明，未改 installed 包 / pnpm store）。developer-id 签名 + 公证（SV1-SUPPLY-03）、release manifest hash pin（RT-DIST-01）、installer 集成仍属 R5。

> _Changelog (issue #36)_：pnpm side-effects 缓存会把 postinstall 产生的符号链接恢复为实体拷贝（上游 [pnpm#12859](https://github.com/pnpm/pnpm/issues/12859)）——warm store 下 fresh worktree 得到的 Electron.app 框架 symlink 全部解引用，fuse fixture 编辑的 `Versions/Current/...` 与加载器解析的顶层路径变成两个不同 inode。仓库对策：`pnpm-workspace.yaml` 设 `sideEffectsCache: false`（本仓库唯一允许构建的 electron@34.5.8 每次 install 重跑 postinstall，保证解包结构新鲜）；`pnpm verify:electron` 增加解包后 bundle 结构断言（`scripts/electron-bundle-layout.mjs`，symlink 缺失/解引用即 exit 1 并给出修复指引），防止已被污染的 store 静默进入 fixture。上游修复后可视情况重新评估缓存收益。
