...
- **RT-TERM-11**：Snapshot 的生成不依赖 Electron 或 Active Attachment。Session Runtime 按需启动兼容版本的 `@xterm/headless`、`@xterm/addon-serialize` 与 `@xterm/addon-unicode11` Snapshot Worker，严格按 seq 回放已校验 chunk，只在 RT-ORDER-08 的安全 checkpoint 写出 app-owned、非 HTML Snapshot 并销毁；Worker 不得取得 PTY fd、Control Lease 或发送 input。
