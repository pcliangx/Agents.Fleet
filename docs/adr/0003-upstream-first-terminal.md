...
终端 package allowlist 仅包含 `@xterm/xterm`、`@xterm/headless`、`@xterm/addon-webgl` 与 `@xterm/addon-serialize`；它们必须锁定兼容的精确版本和包完整性并随签名应用发布，运行时不得从 CDN 或网络加载终端代码。v1 不加载 `addon-image`，image protocol 只按有界、不产生 Host 副作用的未知效果处理。title、OSC 8、clipboard、bell 等效果统一交给私有 `TerminalEffectPolicy` Implementation；escape sequence 不能直接调用 Host 能力。
