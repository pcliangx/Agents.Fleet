...
// bit, architecture, signature and hash, and apply a chmod repair as an owned
// downstream patch (SV1-SUPPLY-02) rather than relying on an npm lifecycle.
//
...
// (stat / file / codesign / sha256) lives in spawn-helper-sampler.ts.
...
/**
 * The chmod 0755 repair is the owned downstream patch (SV1-SUPPLY-02). It is
 * permitted ONLY when the executable bit is the sole problem — never repair a
 * wrong-architecture, tampered (hash mismatch), or under-signed binary, and
 * never when the helper is already launch-ready.
 */
